...
class SpinUp(object):
    ...
    def setupConvergence(self, iniItems):
        ...
        self.minConvForTotlSto = float(
            iniItems.globalOptions['minConvForTotlSto'])
        self.minConvForSoilSto = float(
            iniItems.globalOptions['minConvForSoilSto'])
        self.minConvForGwatSto = float(
            iniItems.globalOptions['minConvForGwatSto'])
        self.minConvForChanSto = float(
            iniItems.globalOptions['minConvForChanSto'])
    ...
    def checkConvergence(self, beginState, endState, spinUpRun, cellAreaMap):
        ...
        beginSoilSto = max(
            1E-20, self.soilStorageVolume(beginState, cellAreaMap))
        ...
        logging.getLogger('spinup').info(
            'Delta GwatStorage = %.2f percent' % (convGwatSto))

        # calculate convergence of channel storage

        beginChanSto = max(
            1E-20, self.channelStorageVolume(beginState, cellAreaMap))
        ...
        beginTotlSto = max(
            1E-20, self.totalStorageVolume(beginState, cellAreaMap))
