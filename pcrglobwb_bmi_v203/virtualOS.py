...
def netcdf2PCRobjCloneWithoutTime(ncFile, varName,
    ...
    if sameClone == False:
        ...
        xIdxSta = int(np.where(
            abs(f.variables['lon'][:] - (xULClone + 0.5*cellsizeInput)) == minX)[0])
        xIdxEnd = int(math.ceil(xIdxSta + colsClone /
                                (cellsizeInput/cellsizeClone)))
        # ; print(minY)
        minY = min(abs(f.variables['lat'][:] - (yULClone - 0.5*cellsizeInput)))
        yIdxSta = int(np.where(
            abs(f.variables['lat'][:] - (yULClone - 0.5*cellsizeInput)) == minY)[0])
            ...
            try:
                ...
                try:
                    idx = nc.date2index(date, nctime, calendar=nctime.calendar,
                                        select='before')
                    msg = "\n"
                    msg += "WARNING related to the netcdf file: " + \
                        str(ncFile)+" ; variable: "+str(varName)+" !!!!!!"+"\n"
                    msg += "No " + \
                        str(dateInput)+" is available. The 'before' option is used while selecting netcdf time."
                    msg += "\n"
                except:
                    idx = nc.date2index(date, nctime, calendar=nctime.calendar,
                                        select='after')
                    ...
                    msg += "WARNING related to the netcdf file: " + \
                        str(ncFile)+" ; variable: "+str(varName)+" !!!!!!"+"\n"
                    msg += "No " + \
                        str(dateInput)+" is available. The 'after' option is used while selecting netcdf time."
                    msg += "\n"
                logger.info(msg)
                logger.info(msg)
...
def readPCRmapClone(v, cloneMapFileName, tmpDir, absolutePath=None, isLddMap=False, cover=None, isNomMap=False):
    ...
    if v == "None":
        ...
        if sameClone == True:
            ...
            warp = gdalwarpPCR(v, output, cloneMapFileName,
                               tmpDir, isLddMap, isNomMap)
            ...
            co = 'rm '+str(tmpDir)+'*.*'
            cOut, err = subprocess.Popen(co, stdout=subprocess.PIPE, stderr=open(
                '/dev/null'), shell=True).communicate()
    else:
...
def gdalwarpPCR(input, output, cloneOut, tmpDir, isLddMap=False, isNominalMap=False):
    ...
    if isLddMap == True:
        co = 'gdal_translate -ot Int32 ' + \
            str(input)+' '+str(tmpDir)+'tmp_inp.tif'
...
def getMapAttributesALL(cloneMap, arcDegree=True):
    ...
    cOut, err = subprocess.Popen(str('mapattr -p %s ' % (cloneMap)),
                                 stdout=subprocess.PIPE, stderr=open(os.devnull), shell=True).communicate()
    if err != None or cOut == []:
...
#    cOut,err = subprocess.Popen(co, stdout=subprocess.PIPE,stderr=open('/dev/null'),shell=True).communicate()
    #cOut,err = subprocess.Popen(str('mapattr -p %s ' %(cloneMap)), stdout=subprocess.PIPE,stderr=open('/dev/null/'),shell=True).communicate()
    cOut, err = subprocess.Popen(co, stdout=subprocess.PIPE, stderr=open(
        os.devnull), shell=True).communicate()
    # print cOut
    if err != None or cOut == []:
        print "Something wrong with mattattr in virtualOS, maybe clone Map does not exist ? "
        sys.exit()
...
def regridData2FinerGrid(rescaleFac, coarse, MV):
    ...
    fine = np.zeros(nr*nc*rescaleFac*rescaleFac).reshape(nr *
                                                         rescaleFac, nc*rescaleFac) + MV
