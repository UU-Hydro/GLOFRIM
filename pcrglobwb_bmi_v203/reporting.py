...
class Reporting(object):
    ...
    def basic_post_processing(self):
        ...
        self.gwNetCapRise = pcr.ifthen(
            self._model.landSurface.gwRecharge < 0.0, self.gwRecharge*(-1.0))
    ...
    def additional_post_processing(self):
        ...
        if "accuRunoff" in self.variables_for_report:
            self.accuRunoff = pcr.catchmenttotal(
                self.runoff * self._model.routing.cellArea, self._model.routing.lddMap) / vos.secondsPerDay()
        ...
        if "accuBaseflow" in self.variables_for_report:
            self.accuBaseflow = pcr.catchmenttotal(
                self.baseflow * self._model.routing.cellArea, self._model.routing.lddMap)
        ...
        if "accuTotalRunoff" in self.variables_for_report:
            self.accuTotalRunoff = pcr.catchmenttotal(
                self.totalRunoff * self._model.routing.cellArea, self._model.routing.lddMap) / vos.secondsPerDay()
        ...
        self.fracSurfaceWaterAllocation = pcr.ifthenelse(
            self.totalGrossDemand < vos.smallNumber, 1.0, self.fracSurfaceWaterAllocation)
        #
        ...
        self.fracNonFossilGroundwaterAllocation = pcr.ifthenelse(
            self.totalGrossDemand < vos.smallNumber, 0.0, self.fracNonFossilGroundwaterAllocation)
        #
        ...
        self.waterBodyStorage = pcr.ifthen(self._model.routing.landmask,
                                           pcr.ifthen(
                                               pcr.scalar(
                                                   self._model.routing.WaterBodies.waterBodyIds) > 0.,
