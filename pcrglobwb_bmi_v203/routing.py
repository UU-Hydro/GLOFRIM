...
class Routing(object):
    ...
    def __init__(self, iniItems, initialConditions, lddMap):
        ...
        # to control runoff being added to discharge
        self.preventRunoffToDischarge = pcr.ifthen(
            self.landmask, pcr.scalar(1.0))
        # to control the fraction of open water
        self.controlDynamicFracWat = pcr.ifthen(self.landmask, pcr.scalar(1.0))
        # fraction of open water from D-Flow FM
        self.waterBodyFractionFM = pcr.ifthen(self.landmask, pcr.scalar(0.0))
        # ----------------------------------------------------------------------------------------------------------------
        ...
        self.limit_num_of_sub_time_steps = max(
            24.0, self.limit_num_of_sub_time_steps)
        ...
        #
        self.limit_num_of_sub_time_steps = np.int(
            self.limit_num_of_sub_time_steps)
        ...
        if 'minCropWaterKC' in iniItems.routingOptions.keys():
            self.minCropWaterKC = float(
                iniItems.routingOptions['minCropWaterKC'])
    ...
    def getICs(self, iniItems, iniConditions=None):
        ...
        if iniConditions == None:
            ...
            self.channelStorage = vos.readPCRmapClone(
                iniItems.routingOptions['channelStorageIni'], self.cloneMap, self.tmpDir, self.inputDir)
            self.readAvlChannelStorage = vos.readPCRmapClone(
                iniItems.routingOptions['readAvlChannelStorageIni'], self.cloneMap, self.tmpDir, self.inputDir)
            self.avgDischarge = vos.readPCRmapClone(
                iniItems.routingOptions['avgDischargeLongIni'], self.cloneMap, self.tmpDir, self.inputDir)
            self.m2tDischarge = vos.readPCRmapClone(
                iniItems.routingOptions['m2tDischargeLongIni'], self.cloneMap, self.tmpDir, self.inputDir)
            self.avgBaseflow = vos.readPCRmapClone(
                iniItems.routingOptions['avgBaseflowLongIni'], self.cloneMap, self.tmpDir, self.inputDir)
            self.riverbedExchange = vos.readPCRmapClone(
                iniItems.routingOptions['riverbedExchangeIni'], self.cloneMap, self.tmpDir, self.inputDir)
            ...
            self.avgDischargeShort = vos.readPCRmapClone(
                iniItems.routingOptions['avgDischargeShortIni'], self.cloneMap, self.tmpDir, self.inputDir)

            # Initial conditions needed for kinematic wave methods
            self.subDischarge = vos.readPCRmapClone(
                iniItems.routingOptions['subDischargeIni'], self.cloneMap, self.tmpDir, self.inputDir)
        ...
        self.avgDischarge = pcr.ifthen(
            self.landmask, pcr.cover(self.avgDischarge,          0.0))
        self.m2tDischarge = pcr.ifthen(
            self.landmask, pcr.cover(self.m2tDischarge,          0.0))
        self.avgDischargeShort = pcr.ifthen(
            self.landmask, pcr.cover(self.avgDischargeShort,     0.0))
        self.avgBaseflow = pcr.ifthen(
            self.landmask, pcr.cover(self.avgBaseflow,           0.0))
        self.riverbedExchange = pcr.ifthen(
            self.landmask, pcr.cover(self.riverbedExchange,      0.0))
        self.subDischarge = pcr.ifthen(
            self.landmask, pcr.cover(self.subDischarge,         0.0))
        ...
        try:
            self.timestepsToAvgDischarge = pcr.mapmaximum(
        ...
        if iniConditions == None:
            ...
            if iniItems.routingOptions['waterBodyStorageIni'] is not 'None':
                ...
                self.waterBodyStorage = pcr.ifthen(
                    self.landmask, self.waterBodyStorage)
            else:
    ...
    def getRoutingParamAvgDischarge(self, avgDischarge, dist2celllength):
        ...
        outlets = pcr.cover(pcr.ifthen(pcr.scalar(
            self.WaterBodies.waterBodyOut) > 0, pcr.boolean(1)), pcr.boolean(0))
    ...
    def accuTravelTime(self):
        ...
        channelStorageThatWillNotMove += self.channelStorage - \
            pcr.rounddown(self.channelStorage)
    ...
    def estimate_length_of_sub_time_step(self):
        ...
        #
        number_of_loops = max(1.0, pcr.cellvalue(pcr.mapmaximum(
            number_of_sub_time_steps), 1)[1])     # minimum number of sub_time_steps = 1
        number_of_loops = int(
            max(self.limit_num_of_sub_time_steps, number_of_loops))
    ...
    def simplifiedKinematicWave(self):
        ...
        channelStorageForRouting = pcr.max(
            0.0, self.channelStorage)                              # unit: m3

        # water height (m)
        self.water_height = channelStorageForRouting / \
            (pcr.max(self.min_fracwat_for_water_height,
        ...
        for i_loop in range(number_of_loops):
            ...
            msg = "sub-daily time step " + \
                str(i_loop+1)+" from "+str(number_of_loops)
            # logger.info(msg)
            ...
            dischargeInitial = pcr.cover(
                waterBodyOutflowInM3PerSec, dischargeInitial)
            ...
            storage_change_in_volume = pcr.upstream(
                self.lddMap, self.subDischarge * length_of_sub_time_step) - self.subDischarge * length_of_sub_time_step
            channelStorageForRouting += storage_change_in_volume
            #
            # route only non negative channelStorage (otherwise stay):
            channelStorageThatWillNotMove += pcr.ifthenelse(
    ...
    def update(self, landSurface, groundwater, currTimeStep, meteo):
        ...
        if self.ActivateCoupling == 'True':
            self.dynamicFracWat = (
                self.dynamicFracWat * self.controlDynamicFracWat) + self.waterBodyFractionFM
        # -----------------------------------------------------------------------------------------------------
        ...
        if self.method == "kinematicWave":
            self.kinematic_wave_update(
                landSurface, groundwater, currTimeStep, meteo)
        ...
        self.readAvlChannelStorage = self.estimate_available_volume_for_abstraction(
            self.channelStorage)
    ...
    def calculate_evaporation(self, landSurface, groundwater, currTimeStep, meteo):
        ...
        self.waterBodyEvaporation = pcr.ifthen(
            self.landmask, self.waterBodyEvaporation)
    ...
    def calculate_exchange_to_groundwater(self, groundwater, currTimeStep):
        ...
        total_groundwater_abstraction = pcr.max(
            0.0, groundwater.nonFossilGroundwaterAbs + groundwater.unmetDemand)   # unit: m
        ...
        self.riverbedExchange = pcr.min(
            self.riverbedExchange, (1.0-factor)*pcr.max(0.0, self.channelStorage))
        self.riverbedExchange = pcr.ifthenelse(
            self.channelStorage < 0.0, 0.0, self.riverbedExchange)
        self.riverbedExchange = pcr.cover(self.riverbedExchange, 0.0)
        self.riverbedExchange = pcr.ifthen(
            self.landmask, self.riverbedExchange)
    ...
    def reduce_unmet_demand(self, landSurface, groundwater, currTimeStep):
        ...
        if self.debugWaterBalance:
            test = pcr.ifthen(potential_unmet_demand <
                              0.0, potential_unmet_demand)
            a, b, c = vos.getMinMaxMean(pcr.scalar(test), True)
            threshold = 1e-3
            if abs(a) > threshold or abs(b) > threshold:
                logger.info(
                    "WARNING !!!!! Water Balance Error. There is negative unmetDemand ... Min %f Max %f Mean %f" % (a, b, c))
        ...
        if landSurface.usingAllocSegments == False and landSurface.limitAbstraction == False:
            ...
            extra_surface_water_abstraction = pcr.ifthen(
                self.landmask, reduction_for_unmetDemand)
        ...
        if landSurface.usingAllocSegments == True and landSurface.limitAbstraction == False:
            ...
            volActWaterAbstract = vos.getValDivZero(
                ...
                segActWaterAbs
            volActWaterAbstract = pcr.min(
                cellAvlWater, volActWaterAbstract)                               # unit: m3
        ...
        # unit: m
        landSurface.actSurfaceWaterAbstract += extra_surface_water_abstraction
        ...
        # unit: m
        landSurface.allocSurfaceWaterAbstract += reduction_for_unmetDemand
        ...
        if self.debugWaterBalance:
            ...
            test = pcr.ifthen(groundwater.unmetDemand <
                              0.0, groundwater.unmetDemand)
            a, b, c = vos.getMinMaxMean(pcr.scalar(test), True)
            threshold = 1e-3
            if abs(a) > threshold or abs(b) > threshold:
                logger.info(
    ...
    def simple_update(self, landSurface, groundwater, currTimeStep, meteo):
        ...
        self.waterBodyOutflow = pcr.cover(
            waterBodyOutflow, 0.0)             # unit: m3/day
        ...
        self.discharge = pcr.max(0., self.discharge)
        ...
        self.disChanWaterBody = pcr.cover(
            self.disChanWaterBody, self.discharge)
        self.disChanWaterBody = pcr.ifthen(
            self.landmask, self.disChanWaterBody)
        #
        # reported channel discharge cannot be negative
        self.disChanWaterBody = pcr.max(0., self.disChanWaterBody)
        #
        #
        ##########################################################################################################################
        ...
        # This option is still EXPERIMENTAL (and not recommended)
        self.allow_extra_evaporation_and_abstraction = False
    ...
    def update_with_extra_evaporation_and_unmet_demand_reduction(self):
        ...
        if groundwater.limitAbstraction == False:
            self.reduce_unmet_demand(landSurface, groundwater, currTimeStep)
    ...
    def calculate_alpha_and_initial_discharge_for_kinematic_wave(self):
        ...
        #
        channel_wetted_area = self.water_height * \
            self.wMean                                  # unit: m2
        channel_wetted_perimeter = 2*self.water_height + \
            self.wMean                                  # unit: m
        #
        alpha = (self.manningsN*channel_wetted_perimeter**(2./3.) *
                 self.gradient**(-0.5))**self.beta  # dimensionless
    ...
    def return_water_body_storage_to_channel(self, channelStorage):
        ...
        channelStorage = pcr.cover(
            waterBodyStoragePerCell, channelStorage)            # unit: m3
    ...
    def kinematic_wave_update(self, landSurface, groundwater, currTimeStep, meteo):
        ...
        for i_loop in range(number_of_loops):
            ...
            if i_loop == 0:
                ...
                acc_water_body_evaporation_volume = pcr.scalar(
                    0.0)   # unit: m3
                acc_water_body_abstraction_volume = pcr.scalar(
                    0.0)   # unit: m3
            ...
            if landSurface.usingAllocSegments == False and landSurface.limitAbstraction == False and\
                ...
                water_body_abstraction_volume = pcr.min(
            ...
            if landSurface.usingAllocSegments == True and landSurface.limitAbstraction == False and \
                ...
                volActWaterAbstract = vos.getValDivZero(
                    ...
                    segActWaterAbs
                water_body_abstraction_volume = pcr.min(
                    cellAvlWater, volActWaterAbstract)
                ...
                if self.debugWaterBalance:
                    ...
                    abstraction = pcr.cover(pcr.areatotal(
                        water_body_abstraction_volume, landSurface.allocSegments)/landSurface.segmentArea, 0.0)
                    allocation = pcr.cover(pcr.areatotal(
                        water_body_allocation_volume, landSurface.allocSegments)/landSurface.segmentArea, 0.0)

                    vos.waterBalanceCheck([pcr.ifthen(self.landmask, abstraction)],
                                          [pcr.ifthen(
                                              self.landmask, allocation)],
                                          [pcr.scalar(0.0)],
                                          [pcr.scalar(0.0)],
                                          'extra surface water abstraction - allocation per zone/segment (PS: Error here may be caused by rounding error.)',
                                          True,
            ...
            storageAtLakeAndReservoirs = pcr.cover(
                storageAtLakeAndReservoirs, 0.0)
            #
            # - move only non negative values and use rounddown values
            storageAtLakeAndReservoirs = pcr.max(
                0.00, pcr.rounddown(storageAtLakeAndReservoirs))
        ...
        if self.debugWaterBalance:
            ...
            if abs(a) > threshold or abs(b) > threshold:
                logger.info(
                    "WARNING !!!!! Water balance errors. There is negative unmetDemand ... Min %f Max %f Mean %f" % (a, b, c))
    ...
    def calculate_statistics(self, groundwater):
        ...
        self.m2tDischarge = self.m2tDischarge + \
            pcr.abs(deltaAnoDischarge*(dishargeUsed - self.avgDischarge))
        #
        ...
        self.avgDischargeShort = self.avgDischargeShort +\
            ...
            pcr.min(self.maxTimestepsToAvgDischargeShort,
                    self.timestepsToAvgDischarge)
    ...
    def estimate_discharge_for_environmental_flow(self, channelStorage):
        ...
        minDischargeForEnvironmentalFlow = pcr.max(
            0.001, self.avgDischarge - 3.5*stdDischarge)
        factor = 0.10  # to avoid flip flop
        minDischargeForEnvironmentalFlow = pcr.max(
            factor*self.avgDischarge, minDischargeForEnvironmentalFlow)   # unit: m3/s
    ...
    def estimate_available_volume_for_abstraction(self, channelStorage):
        ...
        readAvlChannelStorage = safety_factor * \
            pcr.max(0.0, readAvlChannelStorage)

        # ignore small volume values - less than 1 m3
        readAvlChannelStorage = pcr.rounddown(readAvlChannelStorage*1.)/1.
        readAvlChannelStorage = pcr.ifthen(
            self.landmask, readAvlChannelStorage)
