...
class PCRGlobWB(object):
    ...
    def createSubmodels(self, initialState):
        ...
        self.routing = routing.Routing(
            self._configuration, initialState, self.lddMap)
    ...
    def report(self, storesAtBeginning, storesAtEnd):
        ...
        if self._modelTime.doy == 1 or self._modelTime.isFirstTimestep():
            ...
            self.storageAtFirstDay = pcr.ifthen(
                self.landmask, storesAtBeginning)
    ...
    def checkWaterBalance(self, storesAtBeginning, storesAtEnd):
        ...
        if self.landSurface.usingAllocSegments:
            ...
            segTotalDemand = pcr.areatotal(pcr.cover((irrGrossDemand+nonIrrGrossDemand) * self.routing.cellArea,
                                                     0.0), self.landSurface.allocSegments) / self.landSurface.segmentArea

            segAllocSurfaceWaterAbstract = pcr.areatotal(pcr.cover(
                allocSurfaceWaterAbstract * self.routing.cellArea, 0.0), self.landSurface.allocSegments) / self.landSurface.segmentArea

            segAllocNonFossilGroundwaterAbs = pcr.areatotal(pcr.cover(
                allocNonFossilGroundwaterAbs * self.routing.cellArea, 0.0), self.landSurface.allocSegments) / self.landSurface.segmentArea

            segAllocUnmetDemand = pcr.areatotal(pcr.cover(
                allocUnmetDemand * self.routing.cellArea, 0.0), self.landSurface.allocSegments) / self.landSurface.segmentArea
            ...
            vos.waterBalanceCheck([irrGrossDemand, nonIrrGrossDemand],
                                  [surfaceWaterAbstraction,
                                      nonFossilGroundwaterAbs, unmetDemand],
                                  [pcr.scalar(0.0)],
                                  [pcr.scalar(0.0)],
    ...
    def update(self, report_water_balance=False):
        ...
        self.groundwater.update(
            self.landSurface, self.routing, self._modelTime)
