...
from pcrglobwb_bmi_v203.deterministic_runner import DeterministicRunner
...
class pcrglobwbBMI(object):
    ...
    def spinup(self):
        ...
        """
        spin_up = SpinUp(
            self.configuration)                   # object for spin_up
    ...
    def update(self, dt=-1):
        ...
        if dt == 1:
            ...
            # update model
            self.dynamic_framework = DynamicFramework(
                self.deterministic_runner, self.currenttimestep, self.currenttimestep)
            self.dynamic_framework.setQuiet(True)
            self.dynamic_framework.run()
            ...
            # update model
            self.dynamic_framework = DynamicFramework(
                self.deterministic_runner, self.currenttimestep + (dt-1), self.currenttimestep)
            self.dynamic_framework.setQuiet(True)
            self.dynamic_framework.run()
    ...
    def get_var(self, name, missingValues=-999):
        ...
        if numpy.size(name) == 1:
            ...
            if hasattr(self.deterministic_runner.model.landSurface, name):
                pcrmap = getattr(
                    self.deterministic_runner.model.landSurface, name)
                ...
                pcrmap = getattr(
                    self.deterministic_runner.model.groundwater, name)
            else:
                logger.warn(
                    name + " cannot be found in the model, returning empty list!")
            ...
            if name[0] == 'landSurface':
                ...
                if hasattr(self.deterministic_runner.model.routing, name[1]):
                    pcrmap = getattr(
                        self.deterministic_runner.model.routing, name[1])
            elif name[0] == 'WaterBodies':
                if hasattr(self.deterministic_runner.model.routing.WaterBodies, name[1]):
                    pcrmap = getattr(
                        self.deterministic_runner.model.routing.WaterBodies, name[1])
            ...
            # otherwise check if it is a variable from a landCover type
                ...
                try:
                    if hasattr(self.deterministic_runner.model.landSurface.landCoverObj[name[0]], name[1]):
                        pcrmap = getattr(
                            self.deterministic_runner.model.landSurface.landCoverObj[name[0]], name[1])
                    else:
                        logger.warn(
                            '(' + name[0] + ', ' + name[1] + ") cannot be found in the model, returning empty list!")
                except:
                    logger.warn(
                        '(' + name[0] + ', ' + name[1] + ") cannot be found in the model, returning empty list!")
    ...
    def set_var(self, name, var, missingValues=-999):
        ...
        if numpy.size(name) == 1:
            ...
            if hasattr(self.deterministic_runner.model.groundwater, name):
                ...
                logger.warn(
                    name + " is not defined in the model, doing nothing!")
            ...
            # otherwise check if it is a variable from a landCover type
                ...
                try:
                    if hasattr(self.deterministic_runner.model.landSurface.landCoverObj[name[0]], name[1]):
                        ...
                        logger.warn(
                            '(' + name[0] + ', ' + name[1] + + ") is not defined in the model, doing nothing!")
                except:
                    logger.warn(
                        '(' + name[0] + ', ' + name[1] + + ") is not defined in the model, doing nothing!")
