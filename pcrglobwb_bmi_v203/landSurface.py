...
class LandSurface(object):
    ...
    def __init__(self, iniItems, landmask, initialState=None):
        ...
        #
        if self.numberOfSoilLayers == 3:
            self.mainStates += ['storUpp000005',
                                'storUpp005030', 'storLow030150']
            self.stateVars += self.mainStates
        ...
        self.parameters = parSoilAndTopo.SoilAndTopoParameters(
    ...
    def waterDemandOptions(self, iniItems):
        ...
        try:
            ...
            logger.info(
                "WARNING! Domestic water demand is not included in the calculation.")
        #
        ...
        try:
            ...
            logger.info(
                "WARNING! Industry water demand is not included in the calculation.")
        #
        ...
        try:
            if iniItems.landSurfaceOptions['historicalIrrigationArea'] != "None":
                ...
                logger.info(
                    "Extent of irrigation areas is SAME for EVERY YEAR.")
        except:
        ...
        try:
            if iniItems.landSurfaceOptions['allocationSegmentsForGroundSurfaceWater'] != "None":
                ...
                self.allocSegments = pcr.ifthen(
                    self.landmask, self.allocSegments)
    ...
    def scaleNaturalLandCoverFractions(self):
        ...
        self.landCoverObj['grassland'].fracVegCover = 1.0 - \
            self.landCoverObj['forest'].fracVegCover
        ...
        if abs(a) > threshold or abs(b) > threshold:
            logger.info(
                "total of 'Natural Area' fractions is not equal to 1.0 ... Min %f Max %f Mean %f" % (a, b, c))
    ...
    def scaleModifiedLandCoverFractions(self):
        ...
        if abs(a) > threshold or abs(b) > threshold:
            logger.info(
                "fraction total (from all land cover types) is not equal to 1.0 ... Min %f Max %f Mean %f" % (a, b, c))
    ...
    def obtainNonIrrWaterDemand(self, routing, currTimeStep):
        ...
        if currTimeStep.timeStepPCR == 1 or currTimeStep.day == 1:
            if self.domesticWaterDemandOption:
                ...
                if self.domesticWaterDemandFile.endswith('.nc'):
                    ...
                    grossFileName = self.domesticWaterDemandFile + \
                        "w"+str(currTimeStep.year)+".0"+string_month
                    self.domesticGrossDemand = pcr.cover(
                        vos.readPCRmapClone(grossFileName, self.cloneMap, self.tmpDir), 0.0)
                    nettoFileName = self.domesticWaterDemandFile + \
                        "n"+str(currTimeStep.year)+".0"+string_month
                    self.domesticNettoDemand = pcr.cover(
            ...
            self.domesticGrossDemand = pcr.rounddown(
                self.domesticGrossDemand*routing.cellArea)/routing.cellArea
            self.domesticNettoDemand = pcr.rounddown(
                self.domesticNettoDemand*routing.cellArea)/routing.cellArea
            self.domesticGrossDemand = pcr.rounddown(
                self.domesticGrossDemand*routing.cellArea)/routing.cellArea
            self.domesticNettoDemand = pcr.rounddown(
                self.domesticNettoDemand*routing.cellArea)/routing.cellArea
        ...
        if currTimeStep.timeStepPCR == 1 or currTimeStep.doy == 1:
            if self.industryWaterDemandOption:
                ...
                if self.industryWaterDemandFile.endswith('.nc'):
                    ...
                    grossFileName = self.industryWaterDemandFile + \
                        "w"+str(currTimeStep.year)+".map"
                    self.industryGrossDemand = pcr.cover(
                        vos.readPCRmapClone(grossFileName, self.cloneMap, self.tmpDir), 0.0)
                    nettoFileName = self.industryWaterDemandFile + \
                        "n"+str(currTimeStep.year)+".map"
                    self.industryNettoDemand = pcr.cover(
                        vos.readPCRmapClone(nettoFileName, self.cloneMap, self.tmpDir), 0.0)
            else:
            ...
            # ignore small values of demand (less than 1 m3 and/or 0.0001 m):
            self.industryGrossDemand = pcr.rounddown(
                self.industryGrossDemand*routing.cellArea)/routing.cellArea
            self.industryNettoDemand = pcr.rounddown(
                self.industryNettoDemand*routing.cellArea)/routing.cellArea
            self.industryGrossDemand = pcr.rounddown(
                self.industryGrossDemand*routing.cellArea)/routing.cellArea
            self.industryNettoDemand = pcr.rounddown(
                self.industryNettoDemand*routing.cellArea)/routing.cellArea
        ...
        self.domesticGrossDemand = pcr.ifthen(
            self.landmask, self.domesticGrossDemand)
        self.domesticNettoDemand = pcr.ifthen(
            self.landmask, self.domesticNettoDemand)
        self.industryGrossDemand = pcr.ifthen(
            self.landmask, self.industryGrossDemand)
        self.industryNettoDemand = pcr.ifthen(
            self.landmask, self.industryNettoDemand)
    ...
    def calculateCapRiseFrac(self, groundwater, routing, currTimeStep):
        ...
        dzGroundwater = groundwater.storGroundwater / \
            groundwater.specificYield + self.parameters.maxGWCapRise
            ...
            if routing.includeWaterBodies:
                if routing.WaterBodies.useNetCDF:
                    ...
                    routing.WaterBodies.fracWat = vos.readPCRmapClone(
                        ...
                        str(currTimeStep.year)+".map",
                        self.cloneMap, self.tmpDir, self.inputDir)
        FRACWAT = pcr.cover(FRACWAT, 0.0)

        CRFRAC = pcr.min(1.0, 1.0 - (self.parameters.dzRel0100-dzGroundwater) *
                         0.1 / pcr.max(1e-3, self.parameters.dzRel0100-self.parameters.dzRel0090))
        CRFRAC = pcr.ifthenelse(dzGroundwater < self.parameters.dzRel0090, 0.9 - (self.parameters.dzRel0090 -
                                                                                  dzGroundwater)*0.1 / pcr.max(1e-3, self.parameters.dzRel0090-self.parameters.dzRel0080), CRFRAC)
        CRFRAC = pcr.ifthenelse(dzGroundwater < self.parameters.dzRel0080, 0.8 - (self.parameters.dzRel0080 -
                                                                                  dzGroundwater)*0.1 / pcr.max(1e-3, self.parameters.dzRel0080-self.parameters.dzRel0070), CRFRAC)
        CRFRAC = pcr.ifthenelse(dzGroundwater < self.parameters.dzRel0070, 0.7 - (self.parameters.dzRel0070 -
                                                                                  dzGroundwater)*0.1 / pcr.max(1e-3, self.parameters.dzRel0070-self.parameters.dzRel0060), CRFRAC)
        CRFRAC = pcr.ifthenelse(dzGroundwater < self.parameters.dzRel0060, 0.6 - (self.parameters.dzRel0060 -
                                                                                  dzGroundwater)*0.1 / pcr.max(1e-3, self.parameters.dzRel0060-self.parameters.dzRel0050), CRFRAC)
        CRFRAC = pcr.ifthenelse(dzGroundwater < self.parameters.dzRel0050, 0.5 - (self.parameters.dzRel0050 -
                                                                                  dzGroundwater)*0.1 / pcr.max(1e-3, self.parameters.dzRel0050-self.parameters.dzRel0040), CRFRAC)
        CRFRAC = pcr.ifthenelse(dzGroundwater < self.parameters.dzRel0040, 0.4 - (self.parameters.dzRel0040 -
                                                                                  dzGroundwater)*0.1 / pcr.max(1e-3, self.parameters.dzRel0040-self.parameters.dzRel0030), CRFRAC)
        CRFRAC = pcr.ifthenelse(dzGroundwater < self.parameters.dzRel0030, 0.3 - (self.parameters.dzRel0030 -
                                                                                  dzGroundwater)*0.1 / pcr.max(1e-3, self.parameters.dzRel0030-self.parameters.dzRel0020), CRFRAC)
        CRFRAC = pcr.ifthenelse(dzGroundwater < self.parameters.dzRel0020, 0.2 - (self.parameters.dzRel0020 -
                                                                                  dzGroundwater)*0.1 / pcr.max(1e-3, self.parameters.dzRel0020-self.parameters.dzRel0010), CRFRAC)
        CRFRAC = pcr.ifthenelse(dzGroundwater < self.parameters.dzRel0010, 0.1 - (self.parameters.dzRel0010 -
                                                                                  dzGroundwater)*0.05/pcr.max(1e-3, self.parameters.dzRel0010-self.parameters.dzRel0005), CRFRAC)
        CRFRAC = pcr.ifthenelse(dzGroundwater < self.parameters.dzRel0005, 0.05-(self.parameters.dzRel0005 -
                                                                                 dzGroundwater)*0.04/pcr.max(1e-3, self.parameters.dzRel0005-self.parameters.dzRel0001), CRFRAC)
        CRFRAC = pcr.ifthenelse(dzGroundwater < self.parameters.dzRel0001, 0.01-(
            self.parameters.dzRel0001-dzGroundwater)*0.01/pcr.max(1e-3, self.parameters.dzRel0001), CRFRAC)

        CRFRAC = pcr.ifthenelse(FRACWAT < 1.0, pcr.max(
            0.0, CRFRAC-FRACWAT)/(1-FRACWAT), 0.0)
        #
    ...
    def partitioningGroundSurfaceAbstraction(self, groundwater, routing):
        ...
        averageUpstreamInput = pcr.upstream(
            routing.lddMap, routing.avgDischarge)
        ...
        if self.usingAllocSegments:
            ...
            averageBaseflowInput = pcr.cover(pcr.areaaverage(
                averageBaseflowInput, self.allocSegments), 0.0)
            averageUpstreamInput = pcr.cover(pcr.areamaximum(
                averageUpstreamInput, self.allocSegments), 0.0)
        ...
        swAbstractionFraction = pcr.ifthenelse(
            swAbstractionFraction > 0.7, 1.0, swAbstractionFraction)
        ...
        swAbstractionFraction = pcr.ifthen(
            self.landmask, swAbstractionFraction)
    ...
    def scaleDynamicIrrigation(self, yearInInteger):
        ...
        if yearInInteger < 1960 or yearInInteger > 2010:
            logger.info(
                'Dataset of historical irrigation areas is only available from 1960 to 2010.')
        ...
        self.irrigationArea = pcr.min(
            self.irrigationArea, self.cellArea)   # limited by cellArea
        ...
        for coverType in self.coverTypes:
            if coverType.startswith('irr'):
                ...
                # unit: m2
                self.landCoverObj[coverType].fractionArea = self.landCoverObj[coverType].irrTypeFracOverIrr * self.irrigationArea
                self.landCoverObj[coverType].fracVegCover = pcr.min(
                    1.0, self.landCoverObj[coverType].fractionArea / self.cellArea)

                # avoid small values
                self.landCoverObj[coverType].fracVegCover = pcr.rounddown(
                    self.landCoverObj[coverType].fracVegCover * 1000.)/1000.
    ...
    def update(self, meteo, groundwater, routing, currTimeStep):
        ...
        if self.dynamicIrrigationArea and self.includeIrrigation and currTimeStep.doy == 1:
            ...
            for var in self.mainStates:
                ...
                # total states that will be transferred
                ...
                for coverType in self.coverTypes:
                    ...
                    moving_states += pcr.max(0.0, old_fraction-new_fraction) * \
                ...
                if abs(a) > threshold or abs(b) > threshold:
                    logger.info(
                        "WARNING !!!!! Error in transfering states (due to dynamic in land cover fractions) ... Min %f Max %f Mean %f" % (a, b, c))
                else:
                    pass
        ...
        # calculate cell fraction influenced by capillary rise:
        self.capRiseFrac = self.calculateCapRiseFrac(
        ...
        if self.numberOfSoilLayers == 3:
            self.satDegUpp000005 = vos.getValDivZero(
                ...
                vos.smallNumber, 0.)
            self.satDegUpp000005 = pcr.ifthen(
                self.landmask, self.satDegUpp000005)
            self.satDegUpp005030 = vos.getValDivZero(
                self.storUpp005030, self.parameters.storCapUpp005030,
                vos.smallNumber, 0.)
            self.satDegUpp005030 = pcr.ifthen(
                self.landmask, self.satDegUpp005030)
            self.satDegLow030150 = vos.getValDivZero(
                self.storLow030150, self.parameters.storCapLow030150,
                vos.smallNumber, 0.)
            self.satDegLow030150 = pcr.ifthen(
                self.landmask, self.satDegLow030150)
            ...
            self.satDegUppTotal = vos.getValDivZero(
                ...
                vos.smallNumber, 0.)
            self.satDegUppTotal = pcr.ifthen(
                self.landmask, self.satDegUppTotal)
            self.satDegLowTotal = self.satDegLow030150

        # old-style reporting
        self.old_style_land_surface_reporting(
            currTimeStep)             # TODO: remove this one
