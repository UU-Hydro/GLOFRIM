...
class WaterBodies(object):
    ...
    def getICs(self, initial_condition):
        ...
        self.waterBodyStorage = pcr.cover(
            waterBodyStorage, 0.0)        # unit: m3
    ...
    def update(self, newStorageAtLakeAndReservoirs,
        ...
        self.timestepsToAvgDischarge = timestepsToAvgDischarge
    ...
    def getWaterBodyOutflow(self,
        ...
        lakeOutflow = self.getLakeOutflow(
        ...
        # use round values
        self.waterBodyOutflow = pcr.rounddown(
            self.waterBodyOutflow/1.)*1.        # unit: m3
    ...
    def getLakeOutflow(self,
        ...
        #
        lakeOutflow = pcr.ifthen(pcr.scalar(
            self.waterBodyIds) > 0., lakeOutflow)
        lakeOutflow = pcr.ifthen(pcr.scalar(
            self.waterBodyTyp) == 1, lakeOutflow)
    ...
    def getReservoirOutflow(self,
        ...
        #
        resvOutflow = reductionFactor * avgOutflow * \
            length_of_time_step                      # unit: m3

        # maximum release <= average inflow (especially during dry condition)
        resvOutflow = pcr.max(0, pcr.min(
            resvOutflow, self.avgInflow * length_of_time_step))  # unit: m3
        ...
        resvOutflow = pcr.ifthen(pcr.scalar(
            self.waterBodyIds) > 0., resvOutflow)
        resvOutflow = pcr.ifthen(pcr.scalar(
            self.waterBodyTyp) == 2, resvOutflow)
