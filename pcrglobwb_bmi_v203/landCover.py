...
class LandCover(object):
    ...
    def __init__(self, iniItems, nameOfSectionInIniFile, parameters, landmask, usingAllocSegments=False):
        ...
        self.ActivateZeroInfiltrationFloodplain = iniItems.globalOptions[
            'ActivateZeroInfiltrationFloodplain']
        self.ActivateFactorInfiltrationFloodplain = iniItems.globalOptions[
            'ActivateFactorInfiltrationFloodplain']

        # introduce relevant variables for 2-way coupling so they can be changed through BMI when needed
        self.floodplainWaterLayer = pcr.ifthen(
            self.landmask, pcr.scalar(0.0))  # layer of water from D-Flow FM
        # fraction of inundated area from D-Flow FM
        self.inundatedFraction = pcr.ifthen(self.landmask, pcr.scalar(0.0))
        # to control where the floodplain infiltration scaling factor is applied
        self.controlFloodplainFactor = pcr.ifthen(
        ...
        # direct runoff to be calculated from floodplainWaterLayer
        self.directRunoffFloodplain = pcr.spatial(pcr.scalar(0.))
        # evaporation to be calculated from floodplainWaterLayer
        self.openWaterEvapFloodplain = pcr.spatial(pcr.scalar(0.))
        # infiltration to be calculated from floodplainWaterLayer
        self.infiltrationFloodplain = pcr.spatial(pcr.scalar(0.))
        # ----------------------------------------------------------------------------------------------------------------
                ...
                for var in self.outMonthTotNC:
                    ...
                    if currTimeStep.endMonth == True:
                        self.netcdfObj.data2NetCDF(str(self.outNCDir)+"/" +
                                                   ...
                                                   "monthTot.nc",
                                                   var,
                                                   pcr2numpy(self.__getattribute__(
                                                       var+'Tot'), vos.MV),
    ...
    def getPotET(self, meteo, currTimeStep):
        ...
        self.fracPotBareSoilEvap = vos.getValDivZero(
            self.potBareSoilEvap, self.totalPotET, vos.smallNumber)
    ...
    def interceptionUpdate(self, meteo, currTimeStep):
        ...
        self.interceptStor = self.interceptStor - self.interceptEvap
    ...
    def getSoilStates(self, parameters):
        ...
        if self.numberOfLayers == 2:
            ...
            self.effSatUpp = pcr.max(0., self.storUpp / parameters.storCapUpp)
            # THEFF2= max(0,S2_L[TYPE]/SC2[TYPE]);
            self.effSatLow = pcr.max(0., self.storLow / parameters.storCapLow)
            ...
            self.matricSuctionLow = parameters.airEntryValueLow *\
                (pcr.max(0.01, self.effSatLow)**-
                 parameters.poreSizeBetaLow)       # PSI1= PSI_A1[TYPE]*max(0.01,THEFF1)**-BCH1[TYPE];
        ...
        if self.numberOfLayers == 3:
            ...
            self.effSatUpp000005 = pcr.max(
                0., self.storUpp000005 / parameters.storCapUpp000005)
            self.effSatUpp005030 = pcr.max(
                0., self.storUpp005030 / parameters.storCapUpp005030)
            self.effSatLow030150 = pcr.max(
                0., self.storLow030150 / parameters.storCapLow030150)
            ...
            # matricSuction (m)
            self.matricSuctionUpp000005 = parameters.airEntryValueUpp000005 * \
                (pcr.max(0.01, self.effSatUpp000005)
                 ** -parameters.poreSizeBetaUpp000005)
            self.matricSuctionUpp005030 = parameters.airEntryValueUpp005030 * \
                (pcr.max(0.01, self.effSatUpp005030)
                 ** -parameters.poreSizeBetaUpp005030)
            self.matricSuctionLow030150 = parameters.airEntryValueLow030150 * \
                (pcr.max(0.01, self.effSatLow030150)
                 ** -parameters.poreSizeBetaLow030150)

            # kUnsat (m.day-1): unsaturated hydraulic conductivity
            self.kUnsatUpp000005 = pcr.max(
                0., (self.effSatUpp000005**parameters.campbellBetaUpp000005)*parameters.kSatUpp000005)
            self.kUnsatUpp005030 = pcr.max(
                0., (self.effSatUpp005030**parameters.campbellBetaUpp005030)*parameters.kSatUpp005030)
            self.kUnsatLow030150 = pcr.max(
                0., (self.effSatLow030150**parameters.campbellBetaLow030150)*parameters.kSatLow030150)

            self.kUnsatUpp000005 = pcr.min(
                self.kUnsatUpp000005, parameters.kSatUpp000005)
            self.kUnsatUpp005030 = pcr.min(
                self.kUnsatUpp005030, parameters.kSatUpp005030)
            self.kUnsatLow030150 = pcr.min(
                self.kUnsatLow030150, parameters.kSatLow030150)
    ...
    def calculateWaterDemand(self, parameters, nonIrrGrossDemand, swAbstractionFraction, groundwater, routing, allocSegments, currTimeStep):
        ...
        # TODO: Please check! Do we really have to cover?
        self.nonIrrGrossDemand = pcr.cover(nonIrrGrossDemand, 0.0)
        self.nonIrrGrossDemand = pcr.ifthen(
            self.landmask, self.nonIrrGrossDemand)
        ...
        self.totalPotentialGrossDemand = pcr.cover(
            self.nonIrrGrossDemand + self.irrGrossDemand, 0.0)
        ...
        if self.usingAllocSegments:      # using zone/segment at which supply network is defined
            ...
            segAvlSurfaceWater = pcr.areatotal(
                cellAvlSurfaceWater, allocSegments)
            ...
            volActSurfaceWaterAbstract = vos.getValDivZero(
                ...
                segActSurWaterAbs
            volActSurfaceWaterAbstract = pcr.min(
                cellAvlSurfaceWater, volActSurfaceWaterAbstract)  # unit: m3
            ...
            if self.debugWaterBalance == str('True'):
                ...
                abstraction = pcr.cover(pcr.areatotal(
                    self.actSurfaceWaterAbstract * routing.cellArea, self.allocSegments)/self.segmentArea, 0.0)
                allocation = pcr.cover(pcr.areatotal(
                    self.allocSurfaceWaterAbstract*routing.cellArea, self.allocSegments)/self.segmentArea, 0.0)
        ...
        self.potGroundwaterAbstract = pcr.max(
            0.0, self.totalPotentialGrossDemand - self.allocSurfaceWaterAbstract)              # unit: m
        ...
        if self.limitAbstraction:
            ...
            #  -- avoid small values
            readAvlStorGroundwater = pcr.rounddown(
                groundwater.storGroundwater*10000.)/10000.
            ...
            self.potGroundwaterAbstract = self.nonIrrGrossDemand + \
                self.irrGrossDemand - self.allocSurfaceWaterAbstract
        ...
        if self.limitAbstraction == False and\
           groundwater.limitFossilGroundwaterAbstraction and groundwater.usingAllocSegments == False:
            ...
            allocRenewableAvlWater = pcr.min(
                renewableAvlWater, self.totalPotentialGrossDemand)
            ...
            self.potGroundwaterAbstract = self.totalPotentialGrossDemand - \
                self.allocSurfaceWaterAbstract
        ...
        if groundwater.limitRegionalAnnualGroundwaterAbstraction:
            ...
            reductionFactorForPotGroundwaterAbstract = pcr.rounddown(
                reductionFactorForPotGroundwaterAbstract*100.)/100.
            reductionFactorForPotGroundwaterAbstract = pcr.min(
                1.00, reductionFactorForPotGroundwaterAbstract)
    ...
    def calculateDirectRunoff(self, parameters):
        ...
        self.topWaterLayer += pcr.max(0.,
                                      self.netLqWaterToSoil + self.irrGrossDemand)
        ...
        if self.ActivateCoupling == 'True':
            ...
            self.directRunoff = self.directRunoff * \
                pcr.max(0., (1 - self.inundatedFraction))
            ...
            self.directRunoffFloodplain = self.improvedArnoScheme(
                ...
                parameters=parameters)
            self.directRunoffFloodplain = pcr.min(
                self.floodplainWaterLayer, self.directRunoffFloodplain)
            # Yet, no directRunoff in the paddy field.
            if self.name == 'irrPaddy':
                self.directRunoff = 0.
            ...
            self.directRunoffFloodplain = self.directRunoffFloodplain * \
    ...
    def improvedArnoScheme(self, iniWaterStorage, inputNetLqWaterToSoil, parameters, directRunoffReduction="Default"):
        ...
        WFRAC = pcr.min(1.0, DW/self.rootZoneWaterStorageRange)
        # WFRACB = WFRAC**(1/(1+BCF[TYPE]));
        self.WFRACB = WFRAC**(1./(1.+self.arnoBeta))
        #
        ...
        if directRunoffReduction == "Default":
            if self.numberOfLayers == 2:
                directRunoffReduction = pcr.min(self.kUnsatLow,
                                                pcr.sqrt(parameters.kUnsatAtFieldCapLow *
                                                         self.kUnsatLow))
            if self.numberOfLayers == 3:
                directRunoffReduction = pcr.min(self.kUnsatLow030150,
        ...
        # directRunoff
        condition = (self.arnoBeta+pcr.scalar(1.)) * \
    ...
    def calculateOpenWaterEvap(self):
        ...
        self.topWaterLayer = pcr.max(
            0., self.topWaterLayer - self.openWaterEvap)
        ...
        if self.ActivateCoupling == 'True':
            ...
            self.openWaterEvap = self.openWaterEvap * \
                pcr.max(0., (1 - self.inundatedFraction))
            ...
            self.floodplainWaterLayer = pcr.max(
                0., self.floodplainWaterLayer - self.openWaterEvapFloodplain)

            # scale openWaterEvap from floodplainWaterlayer based on inundated fraction
            self.openWaterEvapFloodplain = self.openWaterEvapFloodplain * \
                (self.inundatedFraction)
    ...
    def calculateInfiltration(self, parameters):
        ...
        if self.numberOfLayers == 2:
            ...
            self.infiltration = pcr.min(self.topWaterLayer, parameters.kSatUpp)

        if self.numberOfLayers == 3:
            # P0_L = min(P0_L,KS1*Duration*timeslice());
            self.infiltration = pcr.min(
                self.topWaterLayer, parameters.kSatUpp000005)
        ...
        if self.ActivateCoupling == 'True':
            ...
            if self.ActivateZeroInfiltrationFloodplain == 'False':
                ...
                if self.ActivateFactorInfiltrationFloodplain == 'True':
                    ...
                    self.factorInfiltrationFloodplain = pcr.max(
                        pcr.max(0., (1 - self.satAreaFrac)), self.controlFloodplainFactor)
                    # calculate infiltration
                    if self.numberOfLayers == 2:
                        self.infiltrationFloodplain = pcr.min(
                            self.floodplainWaterLayer, parameters.kSatUpp * self.factorInfiltrationFloodplain)
                    if self.numberOfLayers == 3:
                        self.infiltrationFloodplain = pcr.min(
                            self.floodplainWaterLayer, parameters.kSatUpp000005 * self.factorInfiltrationFloodplain)
                else:
                    if self.numberOfLayers == 2:
                        self.infiltrationFloodplain = pcr.min(
                            self.floodplainWaterLayer, parameters.kSatUpp)
                    if self.numberOfLayers == 3:
                        self.infiltrationFloodplain = pcr.min(
                            self.floodplainWaterLayer, parameters.kSatUpp000005)

                # update floodplainWaterLayer layer after infiltration
                self.floodplainWaterLayer = pcr.max(
                    0., self.floodplainWaterLayer - self.infiltrationFloodplain)

                # scale infiltration from floodplainWaterlayer based on inundated fraction
                self.infiltrationFloodplain = self.infiltrationFloodplain * \
                    (self.inundatedFraction)
                ...
                self.topWaterLayer = self.topWaterLayer * \
                    pcr.max(0., (1 - self.inundatedFraction)) + \
                    self.floodplainWaterLayer * self.inundatedFraction
